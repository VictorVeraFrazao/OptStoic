...
Optimizes an previously built optstoic model. Returns optimized model `os_model` and a dictionary with the optimized stoichiometry `res_dc` if `return_solution` is `true`.
"""
function OptStoic(
    os_model, 
